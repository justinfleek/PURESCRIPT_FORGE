 ...
 * 
 * This plugin integrates Forge with the Bridge Server, forwarding all events
 * to enable real-time sidepanel updates.
 * 
 * Spec: 21-PLUGIN-SYSTEM
 */
import type { Hooks, PluginInput, Event } from "@forge-ai/plugin";
import { BridgeClient } from "./bridge-client.js";
import pino from "pino";
import type { ForgeEvent, ForgeConfig } from "./forge-types.js";

const logger = pino({ name: "forge-sidepanel-plugin" });

/**
 * Forge Sidepanel Plugin
 * 
 * Connects to Bridge Server and forwards all Forge events
 */
export async function SidepanelPlugin(input: PluginInput): Promise<Hooks> {
  logger.info("Initializing Forge Sidepanel Plugin");
  ...
  return {
    /**
     * Main event hook - receives all Forge events
     */
    event: async ({ event }: { event: Event }) => {
      if (!connected) return;
      
      try {
        // Forward event to Bridge Server (typed as ForgeEvent)
        await bridgeClient.sendEvent(event as ForgeEvent);
      } catch (error) {
    ...
    config: async (config) => {
      ...
      try {
        await bridgeClient.sendConfig(config as ForgeConfig);
      } catch (error) {
