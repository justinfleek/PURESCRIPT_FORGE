    ...
    return function() {
      try {
        ...
        switch (event.type) {
          ...
          default:
            ...
            if (store && store.logger && store.logger.info) {
              store.logger.info("Unknown OpenCode event type: " + event.type)();
            }
        }
      } catch (e) {
        console.error("Failed to handle OpenCode event:", e);
        if (store !== undefined && store !== null && store.logger !== undefined && store.logger !== null && store.logger.error !== undefined && store.logger.error !== null) {
          ...
          store.logger.error("OpenCode event handling error: " + errorMessage)();
        }
      }
    };
  };
