...
class ThemeGeneratorPanelV2 {
    static createOrShow(context) {
        ...
        const panel = vscode.window.createWebviewPanel(ThemeGeneratorPanelV2.viewType, "Prism Premium Themes", column, {
            enableScripts: true,
    ...
    async _applyPreset(presetId) {
        const preset = presetGallery_1.ALL_PRESETS.find(p => p.id === presetId);
        if (!preset) {
            vscode.window.showErrorMessage(`Preset "${presetId}" not found`);
            return;
        }
        try {
            await (0, themeInstaller_1.installTheme)(this._context, preset);
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to install theme: ${error}`);
        }
    }
    _previewPreset(presetId) {
        const preset = presetGallery_1.ALL_PRESETS.find(p => p.id === presetId);
        if (!preset)
            return;
    ...
    async _generateCustomTheme(config) {
        // Try to call Haskell backend, fallback to preset-based generation
        try {
            const { generateThemeVariants } = await Promise.resolve().then(() => __importStar(require("./ffiBridge")));
            ...
            vscode.window.showWarningMessage("Backend not available. Using preset-based generation.");
        }
    }
    _update() {
    ...
    _getHtmlForWebview() {
        ...
        const generatePresetCards = (presets, category) => {
...
<body class="${effectsEnabled ? 'effects-on' : ''}">
  ...
  <div class="preset-grid" data-section="luxury">
    ${generatePresetCards(presetGallery_1.LUXURY_PRESETS, 'luxury')}
  </div>
  ...
  <div class="preset-grid" data-section="glass">
    ${generatePresetCards(presetGallery_1.GLASS_PRESETS, 'glass')}
  </div>
  ...
  <div class="preset-grid" data-section="bento">
    ${generatePresetCards(presetGallery_1.BENTO_PRESETS, 'bento')}
  </div>
  ...
  <div class="preset-grid" data-section="neumorphism">
    ${generatePresetCards(presetGallery_1.NEUMORPHISM_PRESETS, 'neumorphism')}
  </div>
  ...
  <div class="preset-grid" data-section="responsive">
    ${generatePresetCards(presetGallery_1.RESPONSIVE_PRESETS, 'responsive')}
  </div>
  ...
  <div class="preset-grid" data-section="easter_eggs">
    ${generatePresetCards(presetGallery_1.EASTER_EGG_PRESETS, 'easter_eggs')}
  </div>
...
exports.ThemeGeneratorPanelV2 = ThemeGeneratorPanelV2;
ThemeGeneratorPanelV2.viewType = "prismThemeGeneratorV2";
//# sourceMappingURL=themeGeneratorPanelV2.js.map