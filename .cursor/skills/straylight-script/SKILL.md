...
description: Write Haskell scripts using straylight-std's Aleph.Script infrastructure (Aleph.Script, Dhall configs, Nix integration). Use when creating Haskell scripts, shell replacements, container operations, VM management, GPU passthrough, or Nix build tooling.
---

# Straylight Script - Aleph.Script Haskell Scripts

Write Haskell scripts using `straylight-std`'s `Aleph.Script` infrastructure instead of bash for complex operations.

## Core Infrastructure

**Aleph.Script** provides a batteries-included prelude combining:
...
Edit `nix/overlays/script.nix`, add to compiled attrset:
...
2. **Use `errExit False`** when a command failing is acceptable
3. **Import tools qualified** to avoid name clashes
4. **Wrap external tools** in `Aleph/Script/Tools/` for reuse
5. **Parse args early**, fail fast with clear usage message
6. **Use `withTmpDir`** for intermediate files
7. **Use builder pattern (`&`)** for Bwrap sandboxes
8. **Patch ELF binaries** with `$ORIGIN` RPATHs for portability
