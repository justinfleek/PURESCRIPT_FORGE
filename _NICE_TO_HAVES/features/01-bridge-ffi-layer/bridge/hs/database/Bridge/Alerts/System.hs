...
evaluateAlerts manager = do
  ...
  where
    isFiring :: Alert -> Bool
    isFiring alert = case alertState alert of
      AlertFiring _ -> True
      _ -> False
...
evaluateRule manager now rule = do
  ...
  metricValue <- getMetricValue manager (arMetric rule)
  
  -- Check if condition met
  let conditionMet = evaluateCondition metricValue (arThreshold rule) (arOperator rule)
  
  -- Get current alert state
  alerts <- readTVar (amAlerts manager)
  let currentAlert = Map.lookup (arName rule) alerts
  
  -- Update alert state
  newState <- case (conditionMet, currentAlert) of
    (True, Just alert) -> do
      case alertState alert of
        AlertFiring _ -> pure (AlertFiring (getFiringTime (alertState alert)))
        _ -> pure (AlertFiring now)
    (True, Nothing) -> pure (AlertFiring now)
    (False, Just alert) -> do
      case alertState alert of
        AlertFiring _ -> pure (AlertResolved now)
        _ -> pure (alertState alert)
  ...
  -- Update alerts map
  modifyTVar' (amAlerts manager) (Map.insert (arName rule) newAlert)
...
sendAlert :: AlertManager -> Alert -> IO (Either String ())
sendAlert manager alert = do
  -- In production, would send via Slack, PagerDuty, email, etc.
  -- For now, log the alert
  putStrLn ("ALERT: " ++ T.unpack (arName (alertRule alert)) ++ " - " ++ T.unpack (arDescription (alertRule alert)))
  pure (Right ())
