...
saveSnapshotFFI (DatabaseHandle conn) snapshotId timestamp stateHash snapshotData trigger description = do
  -- Parse timestamp from ISO string
  timestamp' <- parseUTCTime timestamp
  let snapshot = Snapshot snapshotId timestamp' stateHash snapshotData trigger description
...
saveSessionFFI (DatabaseHandle conn) sessionRecordId sessionId promptTokens completionTokens totalTokens cost model provider startedAt endedAt = do
  startedAt' <- parseUTCTime startedAt
  endedAt' <- case endedAt of
    Just e -> Just <$> parseUTCTime e
    Nothing -> return Nothing
  let session = SessionRecord sessionRecordId sessionId promptTokens completionTokens totalTokens cost model provider startedAt' endedAt'
...
parseUTCTime t = do
  ...
  case parsed of
    ...
    Nothing -> fail ("Failed to parse timestamp: " ++ T.unpack t)
