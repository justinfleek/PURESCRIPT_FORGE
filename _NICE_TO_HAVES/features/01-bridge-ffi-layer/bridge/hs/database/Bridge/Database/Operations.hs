...
-- | ToRow instance for ProofRecord (13 fields - must use explicit list since ToRow only supports up to 10-tuples)
instance ToRow ProofRecord where
  toRow proof = 
    [ toField (proofId proof)
    , toField (proofTheoremName proof)
    , toField (proofFilePath proof)
    , toField (proofLineStart proof)
    , toField (proofLineEnd proof)
    , toField (proofStatement proof)
    , toField (proofText proof)
    , toField (proofStatus proof)
    , toField (proofVerifiedAt proof)
    , toField (proofVerifiedBy proof)
    , toField (proofDependencies proof)
    , toField (proofTags proof)
    , toField (proofDescription proof)
    ]
...
deleteSnapshot conn snapshotIdParam = do
  execute conn "DELETE FROM snapshots WHERE id = ?" (Only snapshotIdParam)
...
saveProof conn proof = do
  execute conn
    ...
    proof  -- uses the ToRow instance we defined above
...
listProofs conn statusFilter fileFilter limit offset = do
  let limitVal = maybe 100 id limit
      ...
      queryText = "SELECT id, theorem_name, file_path, line_start, line_end, statement, proof_text, status, verified_at, verified_by, dependencies, tags, description FROM proofs " <> whereClause <> " ORDER BY verified_at DESC LIMIT ? OFFSET ?"
  query conn (Query queryText) (limitVal, offsetVal)
...
deleteProof conn proofIdParam = do
  execute conn "DELETE FROM proofs WHERE id = ?" (Only proofIdParam)
  rowsChanged <- changes conn
  return (rowsChanged > 0)
