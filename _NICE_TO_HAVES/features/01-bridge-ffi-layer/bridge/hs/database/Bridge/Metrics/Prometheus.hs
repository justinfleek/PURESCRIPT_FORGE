...
exportMetrics registry = do
  ...
  let metricLines = Map.foldMapWithKey formatMetric metrics
  pure (T.unlines metricLines)
  where
    formatMetric :: T.Text -> MetricEntry -> [T.Text]
    formatMetric _key (name, help, labels, value) =
      let helpLine = "# HELP " <> name <> " " <> help
          typeLine = case value of
            CounterValue _ -> "# TYPE " <> name <> " counter"
            GaugeValue _ -> "# TYPE " <> name <> " gauge"
            HistogramValue _ -> "# TYPE " <> name <> " histogram"
          valueLine = case value of
            CounterValue v -> name <> "{" <> labels <> "} " <> T.pack (show v)
            GaugeValue v -> name <> "{" <> labels <> "} " <> T.pack (show v)
            HistogramValue buckets ->
              let bucketCount = length buckets
                  bucketSum = sum buckets
              in name <> "{" <> labels <> ",le=\"+Inf\"} " <> T.pack (show bucketCount) <> "\n" <>
                 name <> "_sum{" <> labels <> "} " <> T.pack (show bucketSum) <> "\n" <>
                 name <> "_count{" <> labels <> "} " <> T.pack (show bucketCount)
      in [helpLine, typeLine, valueLine]
