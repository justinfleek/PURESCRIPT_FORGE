...
use vm_memory::{
    Address, GuestAddress, GuestAddressSpace, GuestMemory, GuestMemoryBackend, GuestUsize,
    Permissions,
};
    ...
    M::M: GuestMemoryBackend,
{
    fn map(&self, iova: u64, gpa: u64, size: u64) -> std::result::Result<(), io::Error> {
        ...
        let user_addr = if GuestMemoryBackend::check_range(&*mem, guest_addr, usize_size) {
            match GuestMemoryBackend::get_slice(&*mem, guest_addr, usize_size) {
                Ok(t) => {
