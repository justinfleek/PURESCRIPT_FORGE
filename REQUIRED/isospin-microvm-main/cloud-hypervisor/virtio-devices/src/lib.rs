...
use vm_memory::{GuestAddress, GuestMemory, GuestMemoryBackend};
...
pub fn get_host_address_range<M: GuestMemory + GuestMemoryBackend + ?Sized>(
    mem: &M,
    addr: GuestAddress,
    size: usize,
) -> Option<*mut u8> {
    // vm-memory 0.18: check_range on GuestMemoryBackend takes 2 args
    if GuestMemoryBackend::check_range(mem, addr, size) {
        let slice = mem.get_slice(addr, size).unwrap();
        assert!(slice.len() >= size);
        // vm-memory 0.18: as_ptr moved to ptr_guard_mut()
