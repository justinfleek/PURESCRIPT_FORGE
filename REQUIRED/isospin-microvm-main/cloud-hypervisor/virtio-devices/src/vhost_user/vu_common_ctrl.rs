...
use vm_memory::{
    Address, FileOffset, GuestAddress, GuestMemory, GuestMemoryBackend, GuestMemoryError as MmapError, GuestMemoryRegion,
};
    ...
    pub fn update_mem_table(&mut self, mem: &GuestMemoryMmap) -> Result<()> {
        ...
        for region in mem.iter() {
            let (mmap_handle, mmap_offset) = match region.file_offset() {
                ...
                // vm-memory 0.18: NoMemoryRegion removed, use InvalidBackendAddress
                None => return Err(Error::VhostUserMemoryRegion(MmapError::InvalidBackendAddress)),
            };
