...
use vm_memory::{
    ...
    GuestMemoryBackend, GuestMemoryError, GuestMemoryError as MmapError, GuestMemoryRegion, GuestUsize, MmapRegion, ReadVolatile,
};
...
impl MemoryManager {
    ...
    pub fn new(
        ...
        let (
            ...
            // vm-memory 0.18: from_arc_regions returns GuestRegionCollectionError
            let guest_memory =
                GuestMemoryMmap::from_arc_regions(regions).map_err(|_| Error::GuestMemory(GuestMemoryError::InvalidBackendAddress))?;
            ...
            let mut guest_memory =
                GuestMemoryMmap::from_arc_regions(mem_regions).map_err(|_| Error::GuestMemory(GuestMemoryError::InvalidBackendAddress))?;
                ...
                if let Some(memory_zone) = memory_zones.get_mut(&zone.id) {
                    if let Some(hotplug_size) = zone.hotplug_size {
                        ...
                        if !user_provided_zones && config.hotplug_method == HotplugMethod::Acpi {
                            ...
                            // vm-memory 0.18: insert_region returns GuestRegionCollectionError
                            guest_memory = guest_memory
                                .insert_region(Arc::clone(&region))
                                .map_err(|_| Error::GuestMemory(GuestMemoryError::InvalidBackendAddress))?;
    ...
    pub fn create_ram_region(
        ...
        // vm-memory 0.18: GuestRegionMmap::new returns Option, not Result
        Ok(Arc::new(
            GuestRegionMmap::new(r, start_addr).ok_or(Error::GuestMemory(GuestMemoryError::InvalidBackendAddress))?,
        ))
    }
