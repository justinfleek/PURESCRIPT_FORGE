...
const FORK_REPO = "forge-ai/zed-extensions"
const UPSTREAM_REPO = "zed-industries/extensions"
const EXTENSION_NAME = "forge"
...
async function main() {
  ...
  // Delete any existing branches for forge updates
  ...
  if (oldBranches.length > 0) {
    console.log(`Found ${oldBranches.length} old branch(es), deleting...`)
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating Forge extension to v${cleanVersion}"`
