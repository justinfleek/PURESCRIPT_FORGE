...
import { createForge, type ToolPart } from "@forge-ai/sdk"
...
console.log("Forge server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await forge.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("Creating new forge session...")
    const { client, server } = forge
    ...
    console.log("Created forge session:", createResult.data.id)
  ...
  console.log("Sending to forge:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("Forge response:", JSON.stringify(result, null, 2))
