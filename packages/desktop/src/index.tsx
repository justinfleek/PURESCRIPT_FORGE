...
import { AppBaseProviders, AppInterface, PlatformProvider, Platform } from "@forge-ai/app"
...
import { Splash } from "@forge-ai/ui/logo"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__FORGE__ ??= {}
  const pending = window.__FORGE__.deepLinks ?? []
  window.__FORGE__.deepLinks = [...pending, ...urls]
...
const createPlatform = (password: Accessor<string | null>): Platform => ({
    ...
    await Promise.resolve()
      .then(() => {
        const notification = new Notification(title, {
          body: description ?? "",
          icon: "https://forge.ai/favicon-96x96-v3.png",
        })
  ...
  fetch: (input, init) => {
    ...
    const addHeader = (headers: Headers, password: string) => {
      headers.append("Authorization", `Basic ${btoa(`forge:${password}`)}`)
    }
      ...
      <AppBaseProviders>
        <ServerGate>
          {(data) => {
            ...
            window.__FORGE__ ??= {}
            window.__FORGE__.serverPassword = data().password ?? undefined
