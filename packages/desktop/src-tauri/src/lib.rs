...
const SETTINGS_STORE: &str = "forge.settings.dat";
...
fn get_sidecar_port() -> u32 {
    option_env!("FORGE_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("FORGE_PORT").ok())
...
fn spawn_sidecar(app: &AppHandle, hostname: &str, port: u32, password: &str) -> CommandChild {
    ...
    .env("FORGE_SERVER_USERNAME", "forge")
...
async fn check_server_health(url: &str, password: Option<&str>) -> bool {
    ...
    if let Some(password) = password {
        req = req.basic_auth("forge", Some(password));
    }
        ...
        .setup(move |app| {
            ...
            let window_builder = WebviewWindowBuilder::from_config(&app, config)
                ...
                .initialization_script(format!(
                    r#"
                      window.__FORGE__ ??= {{}};
                      window.__FORGE__.updaterEnabled = {updater_enabled};
                    "#
                ));
...
async fn spawn_local_server(
    ...
    loop {
        if timestamp.elapsed() > Duration::from_secs(30) {
            break Err(format!(
                "Failed to spawn Forge Server. Logs:\n{}",
