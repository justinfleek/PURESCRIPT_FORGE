...
module Permission.BannedConstructs where
...
checkPattern content path BannedConstruct{..} =
  let
    ...
    matches = zip [1 :: Int ..] lines'
    violations = filter (\(_, line) -> T.isInfixOf (T.pack bcPattern) line) matches
  in
    map (\(lineNum, line) ->
      path <> ":" <> show lineNum <> ": " <> bcName <> " - " <> bcReason <> "\n  " <> T.unpack line
    ) violations
