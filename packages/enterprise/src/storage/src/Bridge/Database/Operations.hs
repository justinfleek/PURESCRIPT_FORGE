...
getSnapshot conn snapshotId' = do
  results <- query conn
...
listSnapshots conn limit offset = do
  let limitVal = maybe 100 Prelude.id limit
      offsetVal = maybe 0 Prelude.id offset
  query conn
...
deleteSnapshot conn snapshotId' = do
  changes <- execute conn "DELETE FROM snapshots WHERE id = ?" (Only snapshotId')
  return (changes > 0)
...
deleteProof conn proofId' = do
  changes <- execute conn "DELETE FROM proofs WHERE id = ?" (Only proofId')
  return (changes > 0)
