...
import Data.Argonaut (encodeJson, decodeJson, Json, jsonParser, stringify, (.:), (.:?))
...
inferStream (TritonClient c) req _onChunk = do
  let url = "http://" <> c.config.host <> ":" <> show c.config.httpPort <>
            "/v2/models/" <> c.config.modelName <> "/infer" <>
            "?stream=true"
  ...
  result <- streamInference url (stringify requestBody)
  pure result
...
decodeTritonResponse json = do
  obj <- decodeJson json
  outputs <- obj .: "outputs"
  outputsArray <- decodeJson outputs
  case Array.head (outputsArray :: Array Json) of
    Nothing -> Left "No outputs in response"
    Just output -> do
      outputObj <- decodeJson output
      dataArr <- outputObj .: "data"
      dataArray <- decodeJson dataArr
      text <- case Array.head (dataArray :: Array Json) of
        Nothing -> Left "No data in output"
        Just t -> decodeJson t

      -- Extract id and model with defaults
      idResult <- obj .:? "id"
      modelResult <- obj .:? "model"
      let responseId = fromMaybe "triton-response" idResult
      let responseModel = fromMaybe "triton" modelResult
