-- | PureScript type definitions for OpenCode Session Status types
-- | Phase 2: Type Safety Layer
-- | Mirrors TypeScript/Zod types from opencode-dev/packages/opencode/src/session/status.ts
module Opencode.Types.SessionStatus where
...
instance decodeJsonSessionStatus :: DecodeJson SessionStatus where
  decodeJson json = do
    ...
    case (type_ :: String) of
      ...
      "busy" -> pure Busy
      _ -> Left (TypeMismatch "Invalid session status type")
