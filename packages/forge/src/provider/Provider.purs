-- | PureScript type definitions for Forge Provider types
-- | Phase 2: Type Safety Layer
-- | Mirrors TypeScript/Zod types from forge-dev/packages/forge/src/provider/
module Forge.Types.Provider where
...
-- Note: ModelInfo is a type alias (record), not a data type
-- Generic instances cannot be derived for type aliases
-- Use encodeJson/decodeJson for serialization instead
