// FFI for Forge.Util.Locale
// 1:1 port from _archive/legacy/opencode-original/opencode-dev/packages/opencode/src/util/locale.ts
...
export const todayTimeOrDateTime = (input) => {
  const date = new Date(input);
  const now = new Date();
  const isToday =
