-- | Omega V1 Responses Route Handler
-- |
-- | Handles OpenAI-compatible responses requests.
-- | Wraps the main omega handler with OpenAI format.
-- |
-- | Source: _OTHER/opencode-original/packages/console/app/src/routes/omega/v1/responses.ts
-- | Migrated: 2026-02-04
module Console.App.Routes.Omega.V1.Responses
  ( handleResponsesPOST
  ) where
...
-- | Handler options for OpenAI format
...
-- | Handle POST request for responses
handleResponsesPOST :: APIEvent -> Aff Response
handleResponsesPOST event = handleOmegaRequest event handlerOptions
