---
title: SDK
description: Type-safe JS client for forge server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The forge JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control forge programmatically.
...
const { client } = await createForge()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `forge.json`, but you can override or add configuration inline:
...
console.log(`Server running at ${forge.server.url}`)
...
If you already have a running instance of forge, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@forge-ai/sdk"
```
