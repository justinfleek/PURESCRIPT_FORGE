...
You can add external tools to Forge using the _Model Context Protocol_, or MCP. Forge supports both local and remote servers.
...
You can define MCP servers in your [Forge Config](https://forge.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="forge.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/forge` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Forge automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Forge will:
...
If the server requires authentication, Forge will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `forge mcp auth <server-name>`.
...
```bash
forge mcp auth my-oauth-server
```
...
```bash
forge mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Forge will store the tokens securely in `~/.local/share/forge/mcp-auth.json`.
...
forge mcp debug my-oauth-server
```
...
Your MCPs are available as tools in Forge, alongside built-in tools. So you can manage them through the Forge config like any other tool.
...
```json title="forge.json" {11, 14-18}
{
  "$schema": "https://forge.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
forge mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect Forge to your Sentry account.
