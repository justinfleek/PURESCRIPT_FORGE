...
Create `.forge/commands/test.md`:

```md title=".forge/commands/test.md"
---
...
You can add custom commands through the Forge config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Forge [config](/docs/config):

```json title="forge.jsonc" {4-12}
{
...
- Global: `~/.config/forge/commands/`
- Per-project: `.forge/commands/`

```markdown title="~/.config/forge/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".forge/commands/component.md"
---
...
```md title=".forge/commands/create-file.md"
---
...
```md title=".forge/commands/analyze-coverage.md"
---
...
```md title=".forge/commands/review-changes.md"
---
...
```md title=".forge/commands/review-component.md"
---
...
forge includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
