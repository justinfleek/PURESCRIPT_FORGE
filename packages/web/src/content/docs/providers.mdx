---
title: Providers
description: Using any LLM provider in Forge.
---

import config from "../../../config.mjs"
export const console = config.console

Forge uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Forge config.
...
in `~/.local/share/forge/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Forge
config.
...
Forge Core is a list of models provided by the Forge team that have been
tested and verified to work well with Forge. [Learn more](/docs/core).

:::tip
If you are new, we recommend starting with Forge Core.
:::

1. Run the `/connect` command in the TUI, select forge, and head to [forge.ai/auth](https://forge.ai/auth).
...
It works like any other provider in Forge and is completely optional to use.
...
To use Amazon Bedrock with Forge:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running forge:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY forge

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile forge

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX forge
   ```
   ...
   For project-specific or persistent configuration, use `forge.json`:

   ```json title="forge.json"
   {
     "$schema": "https://forge.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in Forge is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for forge to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX forge
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your forge config.
...
to store token in forge auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Forge uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Core. To lock Forge
to only use your own GitLab-hosted instance, add the following to your
`forge.json` file. It is also recommended to disable session sharing.
...
More documentation on [forge-gitlab-auth](https://www.npmjs.com/package/@gitlab/forge-gitlab-auth) homepage.

##### Configuration

Customize through `forge.json`:
...
{
  "$schema": "https://forge.ai/config.json",
  "plugin": ["@gitlab/forge-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with forge:
...
To use Google Vertex AI with Forge:
...
2. Set the required environment variables:
   ...
   Set them while running forge.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id forge
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through forge, you can always configure it yourself.
...
```jsonc title="~/.config/forge/forge.jsonc"
{
  "$schema": "https://forge.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [forge-helicone-session](https://github.com/H2Shami/forge-helicone-session) plugin to automatically log each Forge conversation as a session in Helicone.

```bash
npm install -g forge-helicone-session
```
...
```json title="forge.json"
{
  "plugin": ["forge-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Forge conversation listed as a separate session.
...
You can configure forge to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="forge.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://forge.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure forge to use local models through LM Studio.

```json title="forge.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://forge.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure forge to use local models through Ollama.

```json title="forge.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://forge.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Forge:
...
3. Copy the API key for use in Forge.
...
6. **Important**: Before using cloud models in Forge, you must pull the model information locally:
...
Forge Core is a list of tested and verified models provided by the Forge team. [Learn more](/docs/core).

1. Sign in to **<a href={console}>Forge Core</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Forge Core**.

   ```txt
   /connect
   ```

3. Enter your Forge API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your forge config.
...
5. You can also customize them through your forge config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' forge
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group forge
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Forge:
...
You can also customize models through your forge config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with forge. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in forge.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `forge.json` file in your project directory:

   ```json title="forge.json" ""myprovider"" {5-15}
   {
     "$schema": "https://forge.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="forge.json" {9,11,17-20}
{
  "$schema": "https://forge.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Forge to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `forge auth list` to see if the credentials
...
2. For custom providers, check the forge config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your forge config.
