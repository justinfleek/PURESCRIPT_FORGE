---
title: Server
description: Interact with forge server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `forge serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that a forge client can use.
...
```bash
forge serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
forge serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `FORGE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `forge`, or set `FORGE_SERVER_USERNAME` to override it. This applies to both `forge serve` and `forge web`.

```bash
FORGE_SERVER_PASSWORD=your-password forge serve
```
...
When you run `forge` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the forge server to interact with forge programmatically.
:::

This architecture lets forge support multiple clients and allows you to interact with forge programmatically.

You can run `forge serve` to start a standalone server. If you have the
forge TUI running, `forge serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Forge [IDE](/docs/ide) plugins.
...
The forge server exposes the following APIs.
