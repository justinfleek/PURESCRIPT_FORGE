...
With Forge you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Forge uses our own `forge` theme.
...
Forge comes with several built-in themes.
...
- Want Forge to match their terminal's appearance
...
Forge supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/forge/themes/*.json` or `$XDG_CONFIG_HOME/forge/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.forge/themes/*.json`
4. **Current working directory** - Defined in `./.forge/themes/*.json`
...
vim ~/.config/forge/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .forge/themes
vim .forge/themes/my-theme.json
```
...
{
  "$schema": "https://forge.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
