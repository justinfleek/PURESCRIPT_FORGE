---
title: Custom Tools
description: Create tools the LLM can call in forge.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside forge's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.forge/tools/` directory of your project.
- Or globally, by placing them in `~/.config/forge/tools/`.
...
```ts title=".forge/tools/database.ts" {1}
...
```ts title=".forge/tools/project.ts" {8}
import { tool } from "@forge-ai/plugin"
...
```python title=".forge/tools/add.py"
import sys
...
```ts title=".forge/tools/python-add.ts" {10}
import { tool } from "@forge-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".forge/tools/add.py")
