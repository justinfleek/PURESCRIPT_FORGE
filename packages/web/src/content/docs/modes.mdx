...
:::caution
Modes are now configured through the `agent` option in the forge config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in forge allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the forge config.
...
forge comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.forge/plans/*.md` to detail the plan itself
...
Configure modes in your `forge.json` config file:
...
- Global: `~/.config/forge/modes/`
- Project: `.forge/modes/`

```markdown title="~/.config/forge/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global forge config and the project specific config.
...
Create mode files in `.forge/modes/` for project-specific modes or `~/.config/forge/modes/` for global modes:

```markdown title=".forge/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/forge/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
