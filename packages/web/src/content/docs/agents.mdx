...
There are two types of agents in Forge; primary agents and subagents.
...
Forge comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Forge comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Forge comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `forge.json` config file:
...
- Global: `~/.config/forge/agents/`
- Per-project: `.forge/agents/`

```markdown title="~/.config/forge/agents/review.md"
---
...
If no temperature is specified, Forge uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Forge config and the project specific config.
...
The model ID in your Forge config uses the format `provider/model-id`. For example, if you're using [Forge Core](/docs/core), you would use `forge/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="forge.json" {3-6,9-12}
{
  "$schema": "https://forge.ai/config.json",
  "tools": {
    "write": true,
...
```json title="forge.json" {3-5,8-10}
{
  "$schema": "https://forge.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `forge models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/forge-ai/forge).
:::
...
```markdown title="~/.config/forge/agents/docs-writer.md"
---
...
```markdown title="~/.config/forge/agents/security-auditor.md"
---
