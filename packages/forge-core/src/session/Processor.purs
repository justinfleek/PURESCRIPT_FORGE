...
-- |
-- | INVARIANTS (proven in proofs/lean/Forge/Proofs/Session.lean):
...
module Forge.Session.Processor where

import Prelude
import Effect.Aff (Aff, attempt, delay, forkAff, killFiber, Fiber)
...
import Forge.Types.Session (SessionInfo, SessionID, SessionTime)
import Forge.Types.Message (MessageInfo, MessageRole(..))
import Forge.Types.Tool (ToolContext, ToolResult)
import Forge.Types.Permission (PermissionRequest, PermissionReply)
...
processIteration runtime = do
  -- TODO:
  -- 1. Build LLM request with context
  -- 2. Send to provider
  -- 3. Parse response
  -- 4. If text: add to messages, return Complete
  -- 5. If tool calls: check permissions, execute, return WaitingForTool
  pure (Right (Processing { iteration: 1 }))

-- | Execute a tool call
-- | Requires permission check first
executeTool :: ToolCall -> ToolContext -> Aff (Either String ToolResult)
executeTool call ctx = do
  -- TODO:
  -- 1. Look up tool in registry
  -- 2. Check permission (may transition to WaitingForPermission)
  -- 3. Execute tool
  -- 4. Return result
...
buildToolContext runtime =
  ...
  , abort: notImplemented "buildToolContext.abort"  -- AbortSignal
  , callID: Nothing
  , extra: Nothing
