...
-- | using standard model ratios: ~60% prompt, ~40% completion for typical sessions.
-- | For accurate splits, extend SessionSummary to carry promptTokens/completionTokens.
sessionToDataPoint :: SessionSummary -> TokenDataPoint
sessionToDataPoint session =
  let
    total = session.tokenCount
    -- Estimate prompt/completion split using typical 60/40 ratio
    -- This is a reasonable default until SessionSummary carries the split
    estimatedPrompt = (total * 60) / 100
    estimatedCompletion = total - estimatedPrompt
  in
