...
import Sidepanel.FFI.DateTime as DateTimeFfi
...
type State =
  ...
  , isEditing :: Boolean
  , wsClient :: Maybe WS.WSClient
  , currentTimeMs :: Number  -- Current time for relative time display
  }
...
-- | Render recent item
renderRecentItem :: forall m. Number -> RecentAction -> H.ComponentHTML Action () m
renderRecentItem nowMs recent =
  HH.div
    ...
    , HH.span
        ...
        [ HH.text $ formatRecentTime nowMs recent.timestamp ]
    , HH.button
...
-- | Format recent time as relative time string
-- | Takes current time (ms) and action timestamp (ms), returns human-readable relative time
formatRecentTime :: Number -> Number -> String
formatRecentTime nowMs timestamp =
  if timestamp == 0.0 then "just now"
  else
    let
      diffMs = nowMs - timestamp
      diffSeconds = Int.floor (diffMs / 1000.0)
      diffMinutes = diffSeconds / 60
      diffHours = diffMinutes / 60
      diffDays = diffHours / 24
    in
      if diffSeconds < 60 then "just now"
      else if diffMinutes < 60 then show diffMinutes <> " min ago"
      else if diffHours < 24 then show diffHours <> " hours ago"
      else if diffDays < 7 then show diffDays <> " days ago"
      else show (diffDays / 7) <> " weeks ago"
...
handleAction = case _ of
  Initialize -> do
    state <- H.get
    -- Get current time for relative time formatting
    nowMs <- liftEffect DateTimeFfi.getCurrentTimeMs
    -- Calculate context-based suggestions
    let suggestions = calculateSuggestions state.appState
    H.modify_ _ { suggestions = suggestions, currentTimeMs = nowMs }
...
-- | Record action usage with real timestamp
...
recordUsage actionId = do
  ...
  H.modify_ _ { recentActions = newRecent, currentTimeMs = nowMs }
