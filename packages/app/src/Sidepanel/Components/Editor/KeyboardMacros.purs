...
loadMacro stateRef name = do
  state <- liftEffect $ Ref.read stateRef
  let found = Array.find (\(_ /\ macro) -> macro.name == Just name) (Map.toUnfoldable state.macros :: Array _)
...
listMacros stateRef = do
  state <- liftEffect $ Ref.read stateRef
  pure (Array.map (\(_ /\ macro) -> macro) (Map.toUnfoldable state.macros :: Array _))
