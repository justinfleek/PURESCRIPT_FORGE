...
setMark stateRef = do
  ...
  let newMarkRing = Array.take 16 (Array.cons newMark state.markRing)
...
setMarkAt stateRef position = do
  state <- liftEffect $ Ref.read stateRef
  let newMarkRing = Array.take 16 (Array.cons position state.markRing)
  liftEffect $ Ref.write
    ( state
        { mark = Just position
        , markRing = newMarkRing
...
normalizeRegion pos1 pos2 =
  if pos1.file == pos2.file then
    ...
    else if pos1.line == pos2.line && pos1.column <= pos2.column then
      { start: pos1, end: pos2, file: pos1.file }
    else
...
setGlobalMark stateRef name position = do
  ...
  let newGlobalRing = case existingMark of
        Nothing -> Array.take 16 (Array.cons (name /\ position) state.globalMarkRing)
