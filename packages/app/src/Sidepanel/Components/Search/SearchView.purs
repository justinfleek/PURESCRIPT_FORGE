...
-- | Uses DateTime FFI to convert to ISO string, then extracts date parts
formatTimestamp :: DateTime -> String
formatTimestamp dt =
  let
    isoStr = DateTimeFfi.toISOString dt
    -- Extract YYYY-MM-DD from ISO string "YYYY-MM-DDTHH:mm:ss.sssZ"
    datePart = String.take 10 isoStr
    -- Extract month and day for short format
    monthStr = String.take 2 (String.drop 5 isoStr)
    dayStr = String.take 2 (String.drop 8 isoStr)
    monthName = case monthStr of
      "01" -> "Jan"
      "02" -> "Feb"
      "03" -> "Mar"
      "04" -> "Apr"
