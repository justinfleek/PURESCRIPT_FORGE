  ...
  LoadPerformanceData -> do
    ...
    case state.sessionId, state.wsClient of
      Just sessionId, Just client -> do
        ...
        -- Call Bridge API for real performance data
        result <- H.liftAff $ Bridge.getPerformanceData client { sessionId: sessionId }
        case result of
          Right response -> do
            -- Decode slowest operations from JSON
            let slowOps = decodeSlowOperations response.slowestOperations
            let sugs = decodeSuggestions response.suggestions
            let perfData =
                  { sessionId: response.sessionId
                  , totalDuration: response.totalDuration
                  , aiThinkingTime: response.aiThinkingTime
                  , toolExecutionTime: response.toolExecutionTime
                  , networkTime: response.networkTime
                  , totalTokens: response.totalTokens
                  , totalCost: response.totalCost
                  , messageCount: response.messageCount
                  , toolCallCount: response.toolCallCount
                  , events: []  -- Events decoded separately if needed for flame graph
                  , slowestOperations: slowOps
                  , suggestions: sugs
                  }
            H.modify_ _ { performanceData = Just perfData, isLoading = false }
          Left _ ->
...
-- | Decode slowest operations from JSON array
decodeSlowOperations :: Json -> Array SlowOperation
decodeSlowOperations json =
  case A.toArray json of
    Nothing -> []
...
decodeSuggestions :: Json -> Array OptimizationSuggestion
decodeSuggestions json =
  case A.toArray json of
    Nothing -> []
