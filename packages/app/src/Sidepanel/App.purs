...
handleAction = case _ of
  ...
  HandleBalanceTrackerOutput output -> case output of
    ...
    BalanceTracker.RefreshRequested -> do
      -- Refresh balance by fetching full state from Bridge
      state <- H.get
      case state.wsClient of
        Just client -> do
          result <- H.liftAff $ Bridge.getState client {}
          case result of
            Right response -> case response.balance of
              Just balanceData -> do
                -- Convert Bridge BalanceState to Actions.BalanceUpdate
                let balanceUpdate =
                      { diem: Just balanceData.venice.diem
                      , flk: Nothing
                      , usd: Just balanceData.venice.usd
                      , effective: balanceData.venice.effective
            ...
            Left _ -> pure unit
        Nothing -> pure unit
    BalanceTracker.ProviderSelected _pid ->
      -- Provider display preference is handled by BalanceTracker component internally
      pure unit
  ...
  HandleKeyboardNavigationOutput output -> case output of
    KeyboardNavigation.KeyboardActionTriggered action -> case action of
      ...
      KeyboardNavigation.NavigateToRoute route ->
        handleAction (HandleRouteChange route)
      KeyboardNavigation.Refresh -> do
        -- Refresh current view by re-fetching state from Bridge
        state <- H.get
        case state.wsClient of
          Just client -> do
            result <- H.liftAff $ Bridge.getState client {}
      ...
      KeyboardNavigation.Confirm -> do
        -- Context-dependent confirm based on current route
        state <- H.get
        case state.currentRoute of
          Session _ -> pure unit  -- Session panel handles its own confirm
          DiffViewer -> do
            -- Accept all pending diff hunks
            case state.wsClient of
              Just client -> do
                void $ H.liftAff $ Bridge.acceptAllDiffHunks client { file: Nothing }
              Nothing -> pure unit
          _ -> pure unit
      _ ->
  ...
  HandleFileContextViewOutput output -> case output of
    FileContextView.FilesRemoved paths -> do
      -- Remove files from context via Bridge API
      state <- H.get
      case state.wsClient of
        Just client -> do
          void $ H.liftAff $ Bridge.removeFileFromContext client
            { paths: paths
            , sessionId: state.appState.activeSessionId
            }
        Nothing -> pure unit
    FileContextView.FilesAdded paths -> do
      -- Add files to context via Bridge API
      state <- H.get
      case state.wsClient of
        Just client -> do
        ...
        Nothing -> pure unit
    FileContextView.ContextRefreshed -> do
      -- Refresh file context by re-listing from Bridge
      -- The FileContextView component handles its own data display;
      -- this just re-queries to ensure server state is fresh
      state <- H.get
      case state.wsClient of
        Just client ->
          void $ H.liftAff $ Bridge.listFilesInContext client
            { sessionId: state.appState.activeSessionId
            , filter: Nothing
            }
        Nothing -> pure unit
  ...
  HandleDiffViewerOutput output -> case output of
    DiffViewer.HunkAccepted file hunkId -> do
      state <- H.get
      case state.wsClient of
        Just client ->
          void $ H.liftAff $ Bridge.acceptDiffHunk client { file: file, hunkId: hunkId }
        Nothing -> pure unit
    DiffViewer.HunkRejected file hunkId -> do
      state <- H.get
      case state.wsClient of
        Just client ->
          void $ H.liftAff $ Bridge.rejectDiffHunk client { file: file, hunkId: hunkId }
        Nothing -> pure unit
    DiffViewer.AllAcceptedInFile file -> do
      state <- H.get
      case state.wsClient of
        Just client ->
          void $ H.liftAff $ Bridge.acceptAllDiffHunks client { file: Just file }
        Nothing -> pure unit
    DiffViewer.AllRejectedInFile file -> do
      state <- H.get
      case state.wsClient of
        Just client ->
          void $ H.liftAff $ Bridge.rejectAllDiffHunks client { file: Just file }
        Nothing -> pure unit
    DiffViewer.AllAccepted -> do
      state <- H.get
      case state.wsClient of
        Just client ->
          void $ H.liftAff $ Bridge.acceptAllDiffHunks client { file: Nothing }
        Nothing -> pure unit
    DiffViewer.AllRejected -> do
      state <- H.get
      case state.wsClient of
        Just client ->
          void $ H.liftAff $ Bridge.rejectAllDiffHunks client { file: Nothing }
        Nothing -> pure unit
    DiffViewer.FilePreviewed file -> do
      -- Read file content and show in a preview
      state <- H.get
      case state.wsClient of
        Just client -> do
          result <- H.liftAff $ Bridge.readFileContent client { path: file }
          case result of
            Right response -> case response.content of
              Just _ -> pure unit  -- FileContextView handles preview display
              Nothing -> pure unit
            Left _ -> pure unit
        Nothing -> pure unit
  ...
  HandleSessionTabsOutput output -> case output of
    ...
    SessionTabs.NewTabRequested -> do
      ...
      case state.wsClient of
        Just client -> do
          result <- H.liftAff $ Bridge.createNewSession client
            { name: Nothing
            , parentId: Nothing
            , model: Nothing  -- Bridge will use default model
            , provider: Nothing
            }
          case result of
            Right response -> do
  ...
  HandleBranchDialogOutput output -> case output of
    BranchDialog.BranchCreated sessionId messageIndex description -> do
      -- Create branch session via Bridge API (generates real UUID server-side)
      state <- H.get
      let branchTitle = if description == "" then "Branch" else description
      case state.wsClient of
        Just client -> do
          result <- H.liftAff $ Bridge.createNewSession client
            { name: Just branchTitle
            , parentId: Just sessionId
            , model: Nothing
            , provider: Nothing
            }
          case result of
