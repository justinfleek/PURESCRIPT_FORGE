...
-- | Note: This requires proof that retry always returns a result (no infinite loops)
axiom retry_termination_axiom (operation : Nat → IO (Either String α)) (config : RetryConfig) :
...
-- |
-- | **Theorem:** Error recovery strategies are applied correctly
-- | Note: This is a specification of the desired property
axiom error_recovery_axiom (error : BridgeError) (strategy : RecoveryStrategy) (correctRecoveryAction : RecoveryAction) :
