...
exports.base16ToVSCodeTheme = base16ToVSCodeTheme;
exports.installTheme = installTheme;
exports.installAllPresets = installAllPresets;
exports.generatePackageThemeContributions = generatePackageThemeContributions;
 ...
 */
function base16ToVSCodeTheme(name, colors, mode) {
    const { base00, base01, base02, base03, base04, base05, base06, base07, base08, base09, base0A, base0B, base0C, base0D, base0E, base0F } = colors;
    return {
        name,
        type: mode,
        colors: {
 ...
 */
async function installTheme(context, preset) {
    const variant = (0, presetGallery_1.presetToVariant)(preset);
    const theme = base16ToVSCodeTheme(`Prism ${preset.name}`, variant.colors, preset.mode);
    ...
    await vscode.workspace.getConfiguration().update("workbench.colorTheme", `Prism ${preset.name}`, vscode.ConfigurationTarget.Global);
    vscode.window.showInformationMessage(`Theme "${preset.name}" installed and applied!`);
}
/**
 * Generate all preset themes at once
 */
async function installAllPresets(context, presets) {
    const themesDir = path.join(context.extensionPath, "themes");
    if (!fs.existsSync(themesDir)) {
        fs.mkdirSync(themesDir, { recursive: true });
    }
 ...
 */
function generatePackageThemeContributions(presets) {
...
//# sourceMappingURL=themeInstaller.js.map