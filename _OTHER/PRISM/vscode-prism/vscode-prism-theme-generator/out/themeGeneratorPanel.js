...
class ThemeGeneratorPanel {
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel(ThemeGeneratorPanel.viewType, "Prism Theme Generator", viewColumn, {
            enableScripts: true,
            localResourceRoots: [
        ...
        ThemeGeneratorPanel.currentPanel = new ThemeGeneratorPanel(panel, extensionUri);
    }
    ...
    _getHtmlForWebview(webview) {
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, "out", "themeGenerator.js"));
        const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, "out", "themeGenerator.css"));
        return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>
  ...
  <script src="${scriptUri}"></script>
    ...
    async _generateTheme(config) {
        try {
            // Use the verified OKLCH generator (TypeScript port of Haskell/Lean4)
            const variants = (0, prismColor_1.generateVariants)(config);
    ...
    _previewTheme(config) {
        // Generate verified variant for preview
        const variants = (0, prismColor_1.generateVariants)(config);
        const variant = variants.find(v => v.name === 'Tuned') || variants[0];
        themePreviewPanel_1.ThemePreviewPanel.createOrShow(this._extensionUri, variant);
    }
...
exports.ThemeGeneratorPanel = ThemeGeneratorPanel;
ThemeGeneratorPanel.viewType = "prismThemeGenerator";
//# sourceMappingURL=themeGeneratorPanel.js.map