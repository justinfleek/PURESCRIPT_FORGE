...
import { tool } from "@forge-ai/plugin"
...
export default tool({
  ...
  args: {
    ...
    labels: tool.schema
      .array(tool.schema.enum(["nix", "opentui", "perf", "desktop", "core", "docs", "windows"]))
  ...
  async execute(args) {
    ...
    if (args.assignee === "fwang" && !args.labels.includes("core")) {
      throw new Error("Only core issues should be assigned to fwang")
    }
