...
import { createForgeClient } from "@forge-ai/sdk"
...
const { client, server } = createForge()
...
try {
  ...
  console.log("forge session", session.id)
  if (shareId) {
...
function createForge() {
  ...
  const proc = spawn(`forge`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createForgeClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/forge|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/forge` or `/oc`")
  }
}
...
async function assertForgeConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to forge server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.forge.ai" : "https://forge.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.forge.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("forge-github-action")
    response = await fetch("https://api.forge.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/forge" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/forge") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to forge...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "forge-agent[bot]"`
  await $`git config --global user.email "forge-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `forge/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/forge-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[forge session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
